...
const checkExpiredAppointments = async (req, res, next) => {
  try {
    ...
    appointments.forEach((appointment) => {
      ...
      const isWeekend =
        moment(appointment.dateTime).isoWeekday() === 6 || moment(appointment.dateTime).isoWeekday() === 7;
      const isHoliday =
        moment(appointment.dateTime).isSame('2023-01-01', 'day') ||
    ...
    console.log(`Удалено ${result.length} истекших талонов`);
    next();
  } catch (err) {
